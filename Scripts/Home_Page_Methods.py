...
class Home_Page_Methods(environmental_setup):
    ...
    def test_4_check_headers_texts(self):
        driver = self.driver
        driver.get("http://localhost:8080/")
        headers_text_list = [spring_stamp, spring_icon, veter_button,owner_button,error_button, home_button]
        headers_texts_elements = [driver.find_element_by_xpath(e) for e in headers_text_list]
        print(headers_texts_elements)
